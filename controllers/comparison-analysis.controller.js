...
const comparisonAnalysisController = {
    ...
    createReport: async (req, res) => {
        try {
            ...
            const { topicId1, topicId2, poiTitle1, report_data, poiTitle2, userId, startDate, endDate, startDate2, endDate2 } =req.body
        ...
        try {
            ...
            const categoryData = categories.reduce((acc, category) => {
                acc[category.category_title] = {
                    hashtags: category.topic_hash_tags ? category.topic_hash_tags.split(',').map(item => item.trim()) : [],
                    keywords: category.topic_keywords ? category.topic_keywords.split(',').map(item => item.trim()) : [],
                    urls: category.topic_urls ? category.topic_urls.split(',').map(item => item.trim()) : []
                };
                return acc;
            }, {});
                                    ...
                                    aggs: {
                                        emotions: {
                                            ...
                                            aggs: {
                                                docs: {
                                                    top_hits: {
                                                        _source: isGoogleSentimentChart === 'true'
                                                            ? ['predicted_sentiment_value']
                                                            : ['p_message', 'llm_emotion', 'predicted_sentiment_value'],
                                                        size: 5
                                                    }
                                                }
                                            }
                    ...
                    buckets: (bucket.sentiments?.buckets || []).map(sentiment => ({
                        ...
                        emotions: (sentiment.emotions?.buckets || []).map(emotion => ({
                            ...
                            messages: (emotion.docs?.hits?.hits || []).map(doc => ({
                                ...
                                predicted_sentiment_value: doc._source.predicted_sentiment_value
                            }))
                        }))
                    }))
                }
    ...
    getDistributions: async (req, res) => {
        try {
            ...
            if (Object.keys(categoryData).length === 0) {
                return res.json({
                    Facebook:0, Twitter:0, Instagram:0, Youtube:0, Linkedin:0, Pinterest:0, Reddit:0, Tumblr:0, Vimeo:0, Web:0}
                   );
            }
    
            // Flatten all keywords, hashtags, and urls from the processed categories
            const socialMediaData = Object.values(categoryData).flatMap(category => [
                ...(category.keywords || []),
                ...(category.hashtags || []),
                ...(category.urls || [])
            ]);
