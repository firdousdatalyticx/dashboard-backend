...
const buildElasticsearchQuery = (params) => {
  ...
  if (postType && typeMapping[postType] && postTypeSource !== 'GoogleMyBusiness') {
  ...
  if (sentiment && sentiment !== 'undefined' && sentiment !== 'null' && postTypeSource !== 'GoogleMyBusiness') {
  ...
  if (postType === 'postsByDate' || postType === 'postsByDateUNtopic') {
    ...
    if (finalQueryString) must.push({ query_string: { query: finalQueryString } });
  }
  ...
  else if (['satisfactionScore', 'churnProbability', 'Low Score', 'Medium Score', 'High Score', 'Low Probability', 'Medium Probability', 'High Probability'].includes(postType)) {
    if (postTypeData) {
...
function addCategoryFilters(query, selectedCategory, categoryData) {
  if (!selectedCategory || selectedCategory.trim().toLowerCase() === 'all') return;
...
const postsController = {
  ...
  getPosts: async (req, res) => {
    try {
      ...
      const isRatingFilterActive = postTypeSource === 'GoogleMyBusiness' && requestedRatingValue !== null;
      ...
      const isSocialMedia =
        !postTypeSource ||
        ['Twitter', 'Facebook', 'Instagram', 'Youtube', 'Pinterest', 'Reddit', 'LinkedIn', 'Web', 'All'].includes(postTypeSource);
      ...
      if (isSocialMedia && topicId && Object.keys(categoryData).length > 0) {
        if (category && category !== 'all') {
          ...
          console.log(`Applying combined category filters for topicId: ${topicId}`);
      ...
      } else if (!isSocialMedia && category && category !== 'all' && Object.keys(categoryData).length > 0) {
        addCategoryFilters(esQuery.query, category, categoryData);
      }
      ...
      const filteredHits = isRatingFilterActive
        ? hits.filter(hit => {
            const hitRating = hit._source.rating !== undefined ? Number(hit._source.rating) : null;
      ...
      const responseArray = await Promise.all(
        filteredHits.map(async hit => {
          ...
          if (postTypeSource === 'GoogleMyBusiness' && hit._source.rating !== undefined)
      ...
      return res.status(200).json({
        ...
        hits,
        responseArray,
        total: isRatingFilterActive ? responseArray.length : results?.hits?.total?.value || 0,  
        foundRatings: postTypeSource === 'GoogleMyBusiness' ? Array.from(ratingsFound) : undefined,
        filtered: isRatingFilterActive ? `${filteredHits.length}/${hits.length}` : undefined,
