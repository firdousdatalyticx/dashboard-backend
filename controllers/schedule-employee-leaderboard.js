...
const oiaApiCall=async()=>{
  try {
    ...
    const fromDate = from.toISOString().split("T")[0];
    const topicId = 2646;
    const isCreateOrUpdateOIA = true;
    const payload1 = {
      topicId,
      fromDate,
      toDate,
    ...
    // Call First API
    await axios.post(
      `${process.env.BACKEND_SERVER}/social-media/employee-engagement-leaderboard/getEmployeeDataOIA`,
      payload1,
    );
