    ...
    try {
      ...
      const emotionsWithPostsPromises = topEmotions.map(
        async (emotionBucket) => {
          ...
          if (source !== "All") {
            emotionQuery.bool.filter.bool.should.push({
              match_phrase: { source: source },
            });
          } else {
            if (isSpecialTopic) {
          ...
          const allPosts = await fetchAllPostsForEmotion(
            emotionQuery,
            parseInt(maxPostsPerEmotion, 10)
          );
      ...
      const emotionsWithPosts = allEmotionsWithPosts.filter(
        (emotion) => emotion !== null
      );

      // Calculate total count for just the included emotions
      const totalCount = emotionsWithPosts.reduce(
...
const formatPostData = async (hit) => {
  ...
  // Format message text
  let message_text = source.p_message_text
