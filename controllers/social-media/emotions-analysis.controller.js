...
const emotionsController = {
    ...
    getEmotionsAnalysis: async (req, res) => {
        ...
        const ninetyDaysAgo = subDays(now, 90);
            ...
            // Get emotion counts
            const emotionBuckets = countResponse.aggregations?.emotions_count?.buckets || [];
            
            // Format the response with just the emotion counts
            const emotions = emotionBuckets.map(bucket => ({
                name: bucket.key,
            ...
            for (const interval of intervalData) {
                ...
                for (const emotionBucket of (interval.emotions.buckets || [])) {
                    ...
                    try {
                        ...
                        const posts = emotionPostsResponse.hits.hits.map(hit => formatPostData(hit));
                        ...
                        console.error(`Error fetching posts for emotion ${emotionName} in interval ${intervalDate}:`, error);
