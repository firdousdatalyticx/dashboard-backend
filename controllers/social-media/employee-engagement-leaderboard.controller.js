...
const employee_engagement_leaderboardController = {
      ...
      const upsertPromises = data.map((row) => {
        return prisma.employee_engagement_leaderboard.upsert({
          ...
          update: {
            ...
            quality_score: parseFloat(row.quality_score) || 0.0,
            final_engagement_score:
              parseFloat(row.final_engagement_score) || 0.0,
            updated_at: new Date(),
          },
          create: {
            topic_id: topicId,
  ...
  CreateAutomateRecord: async (topicId, data) => {
    try {
      if (!topicId || isNaN(topicId) || !data || !Array.isArray(data)) {
        return res.status(400).json({
          success: false,
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        if (existingRecord) {
          ...
          console.log("Creating new record:", element.name, element.date);
...
UpdateNonCpxCount: async (topicId, data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < employee.data.length; j++) {
      const monthData = employee.data[j];
  ...
  GetTopPerformers: async (req, res) => {
    try {
      ...
      const topPerformers =
        await prisma.employee_engagement_leaderboard.findMany({
          where: {
            topic_id: topicId,
            high_q: 1,
          },
  ...
  GetStatistics: async (req, res) => {
    try {
      ...
      const highQualityCount =
        await prisma.employee_engagement_leaderboard.count({
          where: {
            topic_id: topicId,
            high_q: 1,
          },
      ...
      const mediumQualityCount =
        stats._count.id - highQualityCount - lowQualityCount;
        ...
        data: {
          ...
          qualityDistribution: {
            ...
            percentages: {
              high:
                stats._count.id > 0
                  ? ((highQualityCount / stats._count.id) * 100).toFixed(1)
                  : "0.0",
              medium:
                stats._count.id > 0
                  ? ((mediumQualityCount / stats._count.id) * 100).toFixed(1)
                  : "0.0",
              low:
                stats._count.id > 0
                  ? ((lowQualityCount / stats._count.id) * 100).toFixed(1)
                  : "0.0",
            },
          },
        },
