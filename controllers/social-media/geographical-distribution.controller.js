// geographical-distribution.controller.js
...
const geographicalDistributionController = {
  // 1. Get Geographical Distribution (by u_country)
  getGeographicalDistribution: async (req, res) => {
    try {
      ...
      const buckets = response.aggregations.geographical_distribution.buckets;
  ...
  // 2. Get Geographical Sentiment Distribution (by u_country and predicted_sentiment_value)
  getGeographicalSentimentDistribution: async (req, res) => {
    try {
      const {
        timeSlot,
        fromDate,
      ...
      console.error("Error fetching geographical sentiment distribution:", error);
      return res.status(500).json({ error: "Internal server error" });
    }
  },

  // 3. Get Geographical Emotion Distribution (by u_country and llm_emotion)
  getGeographicalEmotionDistribution: async (req, res) => {
    try {
      const {
        timeSlot,
        fromDate,
      ...
      console.error("Error fetching geographical emotion distribution:", error);
      return res.status(500).json({ error: "Internal server error" });
    }
  },

  // 4. Get Posts for Geographical Distribution
  getGeographicalPosts: async (req, res) => {
    try {
      const {
        timeSlot,
        fromDate,
      ...
      if (country && country !== 'undefined' && country !== 'null' && country !== '') {
        query.bool.must.push({
          match_phrase: { "u_country.keyword": country }
        });
      }
      ...
      console.error("Error fetching geographical posts:", error);
      return res.status(500).json({ error: "Internal server error" });
    }
  },
};
...
module.exports = geographicalDistributionController;
