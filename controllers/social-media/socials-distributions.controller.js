...
return res.json({ posts });
  ...
  getCommentskeywordsDistribution: async (req, res) => {
    try {
      const {
        timeSlot,
        fromDate,
    ...
    try {
      ...
      if (signals) {
        ...
        signalsArray.forEach(signal => {
          const signalStr = String(signal).trim();
          if (signalStr && !["null", "Not Specified", "not specified"].includes(signalStr)) {
            signalsMap[signalStr] = (signalsMap[signalStr] || 0) + 1;
          }
        });
      }
    } catch (_) {}
...
return res.status(200).json({
  success: true,
  phrases: Object.entries(signalsMap)
