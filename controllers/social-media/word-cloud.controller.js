...
const buildWordCloudParams = (options) => {
    ...
    if (sentimentType === 'positive') {
        baseQuery.bool.must[0].query_string.query += ' AND predicted_sentiment_value:"Positive"';
    } else if (sentimentType === 'negative') {
        baseQuery.bool.must[0].query_string.query += ' AND (predicted_sentiment_value:"Negative" OR llm_emotion:("Anger" OR "Frustration" OR "Criticism" OR "Confusion"))';
    }
...
const buildPostsByPhraseParams = (options) => {
    ...
    let phraseField = sentimentType === 'positive' 
